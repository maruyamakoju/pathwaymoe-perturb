% arXiv / bioRxiv preprint. Canonical content source: results/PAPER.md.
...
% TODO: Fill in author name(s) and affiliation(s) before submission.
% Example:
% \author{Koju Maruyama \\ Affiliation \\ \texttt{email@example.com}}
...
(density-matched), generic curated (TRRUST), dense curated (CollecTRI, $4\times$ denser and signed),
data-derived co-expression, and data-derived co-response (LFC correlation); we additionally test
drug$\rightarrow$target mechanistic grounding---measuring effect on OOD DEG-Pearson with bootstrap CIs and paired significance
...
statistics, \textbf{no GRN prior---generic, weighted, dense curated (CollecTRI), or data-derived---significantly
improves real-data OOD prediction} (all $|\Delta\,\text{DEG-Pearson}|\le 0.01$, Holm-$p>0.2$); CollecTRI
is statistically indistinguishable from a random graph at the same density ($\Delta=+0.001$, ns), and
drug$\rightarrow$target mechanistic grounding is likewise null ($\Delta=+0.003$, $p=0.93$). Moreover, under the same
leakage-free protocol the deep PathwayMoE does \textbf{not} robustly beat the linear baselines either:
across three OOD splits it loses to ridge on \texttt{unseen\_cell\_line} ($0.824$ vs $0.868$, Holm-$p\!=\!0.001$),
ties the mean-effect baseline on \texttt{unseen\_both} ($0.581$ vs $0.607$, ns), and shows only a small
non-significant edge on \texttt{unseen\_drug} ($0.629$ vs $0.601$, overlapping CIs). On synthetic data
we localize why: when test perturbations \textbf{share targets} with training, even the \emph{true} GRN
is redundant (true GRN $+0.014$, ns); when targets are \textbf{novel}, the true GRN gives a
meaningful but \textbf{non-significant} gain ($\Delta\!=\!+0.079$, Holm-$p\!=\!0.57$) in a
near-unpredictable regime, and the \textbf{injection mechanism} (hard attention mask vs soft
message-passing) does not matter. We
...
deltas. Two-sided bootstrap $p$-values use a $+1/(B+1)$ continuity correction so the smallest
achievable $p$ at $B\!=\!2000$ is $\approx\!5\times10^{-4}$, not the degenerate $p\!=\!0$ a naive
estimator returns when all bootstrap deltas land on one side. \textbf{Holm--Bonferroni} correction
is then applied; effects are called ``significant \emph{and} meaningful'' only when Holm-$p<0.05$
\emph{and} $|\Delta|\ge0.01$ (pre-registered minimum effect).
...
MoE / GRN = \textbf{CollecTRI} (4$\times$ denser, signed) & 0.624 & [0.543, 0.695] \\
MoE / GRN = CollecTRI weighted (signed bias) & 0.619 & [0.537, 0.691] \\
MoE / GRN = random @ CollecTRI density & 0.624 & [0.548, 0.691] \\
MoE / GRN = none $+$ drug$\rightarrow$target & 0.632 & [0.569, 0.689] \\
\bottomrule
\end{tabular}
\caption{Real Tahoe \texttt{unseen\_drug}: neither GRN quality/density nor drug$\rightarrow$target
mechanistic grounding moves accuracy.}
\end{table}

Planned contrasts (paired cluster bootstrap, Holm-corrected): \textbf{every} GRN variant vs none is
within $\pm0.01$ with Holm-$p>0.2$---none significant, none meaningful ($|\Delta|\ge0.01$). The
data-derived co-response GRN---the variant a leaky analysis would have favoured---is $\Delta=-0.005$
($p=0.95$) vs none. \textbf{Dense, curated, signed CollecTRI closes the ``TRRUST was too sparse''
objection}: with 2{,}193 in-space edges ($4\times$ TRRUST's 539) it is $\Delta=-0.005$ vs none and
$\Delta=+0.001$ (Holm-$p=1.0$) vs a \emph{random} graph at the same density---indistinguishable from
random rewiring; edge sign does not help ($\Delta=-0.005$ weighted vs CollecTRI, ns). A complementary
prior, \textbf{drug$\rightarrow$target mechanistic grounding} (which gene each drug hits; 58/212 drugs
have an in-space target), is likewise null: $\Delta=+0.003$ (CI $[-0.086,+0.091]$, $p=0.93$), despite
a strong in-distribution signal that does not transfer, and a synthetic positive control confirming
the target pathway is functional ($\Delta=+0.25$, $p<0.001$ under target ablation). On
\texttt{unseen\_drug} the deep MoE ($\sim$0.63) shows a small numerical edge over the linear baselines
(0.58--0.60) but with heavily overlapping cluster CIs (MoE 0.629 [0.551,0.697] vs B1 0.601
[0.537,0.655])---not a significant win. The null is summarized across all three regimes in
...
\midrule
\texttt{unseen\_cell\_line} & 10 & 0.850 & \textbf{0.868} & 0.824 & $-0.044$ vs ridge, Holm-$p\!=\!0.001$ (sig) \\
...
\textbf{ground\_truth (true GRN)} & \textbf{0.610} & \textbf{+0.014 (Holm-$p=0.35$)} \\
\bottomrule
\end{tabular}
...
GRN as attention mask & 0.396 [0.270, 0.526] & \textbf{+0.079 (0.57)} \\
...
mechanism does not matter. CPU-eval point estimates (mechanism is sensitive to CUDA atomic-add
ordering — see \S\ref{sec:repro}).}
\end{table}

The true GRN yields a meaningful point-estimate gain ($+0.05$--$0.08$, all above the pre-registered
$|\Delta|\!\geq\!0.01$ threshold) but it is \textbf{not statistically significant}
(Holm-$p\!=\!0.57$): novel targets are intrinsically hard, so between-drug variance is large and
the 39-cluster CIs are wide ($[0.18, 0.53]$). The \textbf{injection mechanism does not matter}
(mask $\approx$ propagation $\approx$ both; mask\,vs\,prop $\Delta\!=\!-0.026$), arguing against
``we just used the wrong mechanism.''

\paragraph{Bottom line.} Where OOD prediction is \emph{feasible}---real Tahoe and synthetic
shared-target---a GRN prior confers \textbf{no} benefit. The GRN \emph{plausibly} helps for novel
targets, a near-unpredictable regime where the $+0.05$--$0.08$ gain is meaningful but not
...
the Tahoe subset). The novel-target positive control, while suggestive ($+0.05$--$0.08$), is
underpowered by the intrinsic difficulty of that regime.

\section{Reproducibility \& availability}\label{sec:repro}
...
63 tests incl.\ a leakage regression test and a checkpoint roundtrip. Split seed fixed; 3 model seeds;
deterministic fp32 eval; cluster-bootstrap 95\% CIs $+$ Holm correction with a $+1/(B+1)$ continuity
correction. Each checkpoint stores an env+data provenance manifest. Real data $=$ 803/3{,}388
Tahoe-100M shards (22.66M cells). The OOD-split re-run used batch 48 (batch 96 exhausts 24\,GB VRAM).
\paragraph{Eval-time determinism.} On Tahoe the CUDA noise floor in DEG-Pearson is $\!\leq\!10^{-3}$ and the
default ``\texttt{--eval-device auto}'' (CUDA) is reported. On the near-zero-variance synthetic
novel-target regime (\S3.4) CUDA \texttt{index\_add\_} atomic-add ordering introduces a $\sim\!0.05$
DEG-Pearson swing per process invocation; that table is computed under
``\texttt{--eval-device cpu}'' (bit-reproducible, $\sim\!15$\,min on the same hardware). See
\texttt{AUDIT.md} item N.

\paragraph{Code \& data.}
Code is available at \url{https://github.com/maruyamakoju/pathwaymoe-perturb}.
The real data subset (22.66M cells from Tahoe-100M) can be rebuilt from the public Tahoe-100M release
using \texttt{python -m pmoe.data.preprocess\_tahoe}.

\section*{Acknowledgements}
Parts of this work, including code development, the leakage audit, and the statistical analysis
pipeline, were developed with the assistance of AI coding tools (Claude, Gemini). All results were
independently verified by the authors; numerical claims were cross-checked against source data files.
Computational experiments were run on a single NVIDIA RTX~4090 (24\,GB VRAM).
