...
is redundant (true GRN $+0.014$, ns); when targets are \textbf{novel}, the true GRN gives a small
positive but \textbf{non-significant} gain ($\Delta\approx+0.02$--$0.03$, Holm-$p\!=\!1.0$) in a
...
GRN as attention mask & 0.342 [0.220, 0.462] & \textbf{+0.029 (1.0)} \\
GRN as soft propagation & 0.330 [0.208, 0.450] & +0.017 (1.0) \\
GRN as mask + propagation & 0.337 [0.214, 0.458] & +0.024 (1.0) \\
\bottomrule
\end{tabular}
\caption{Novel targets: the true GRN gives a small positive but non-significant gain; the injection
mechanism does not matter.}
\end{table}

The true GRN yields a small positive point-estimate gain ($+0.02$--$0.03$, marginally above the
pre-registered $|\Delta|\!\geq\!0.01$ threshold) but it is \textbf{not statistically significant}
(Holm-$p\!=\!1.0$): novel targets are intrinsically hard, so between-drug variance is large and the
39-cluster CIs are wide ($[0.19, 0.46]$). The \textbf{injection mechanism does not matter}
...
shared-target---a GRN prior confers \textbf{no} benefit. The GRN only \emph{weakly hints} at helping
for novel targets, a near-unpredictable regime where the $+0.02$--$0.03$ gain is not significant;
independent of injection mechanism.
...
the Tahoe subset). The novel-target positive control, while suggestive ($+0.02$--$0.03$), is underpowered
by the intrinsic difficulty of that regime.
